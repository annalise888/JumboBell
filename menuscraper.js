...
async function weekdates(currURL, hall) {
    ...
    var today = new Date();
    //today = today.getDate();
    //OK so my goal is to delete everything that I'm not currently uploading 
    ...
    for(var i = 0; i < 7; i++ ) {
        ...
        deleteall();
        await downloadURL(newURL, i, hall, today);
...
async function downloadURL(myurl, i, hall, today) {
    ...
    axios( {url: myurl} )
      .then(response => {
        ...
        return siteparse(html, i, hall, today);
      })
      .catch(console.error);
...
async function siteparse(html, currday, hall, today) {
    ...
    for( i = 0; i < html.length; i ++ ) {
        ...
        if( html[i].includes("LUNCH") ) {
            currmeal = "Lunch"
        //    console.log("Dinner: " + html[i]);
        }
        
        if( html[i].includes("Dinner") ) {
            currmeal = "Dinner"
        //    console.log("Dinner: " + html[i]);
        }
        ...
        if( html[i].includes("shortmenurecipes") ) {
            ...
            food = food[0];
            //console.log(food + "daydate:" + daydate);
            foodlist.push(food);
            currmeallist.push(currmeal);
        }
    }
    ...
    mongoinsert(foodlist,currmeallist,  daydate, currday, hall, today);
...
function mongoinsert(foodlist, currmeallist, daydate, numday, hall, today) {
    ...
    MongoClient.connect(url, {useUnifiedTopology: true}, function(err, db) {
      ...
      for(i = 0; i < foodlist.length; i++) {
          console.log("date: " + daydate + " numdate: " + realnumday + " hall: " + hall + " food: " + foodlist[i] + " meal: " + currmeallist[i] );
          ...
          var newData2 = { "longdate" : daydate, "numdate" : realnumday, "hall": hall, "food" : foodlist[i], "meal" : currmeallist[i] , "insertdate" : today};
